...
experimental repo whose entire history is linear on `main`, so a branch adds a merge step and a stale remote ref for
no review benefit. Commit and push only when asked, as usual — the instruction here is about *where*, not *when*.
